...
\begin{frame}
 \frametitle{Geothermal Darcy Flows}
    \begin{enumerate}[1.]\setcounter{enumi}{10} \scriptsize
       ...
       \item <2-> However it is not really the velocity in which the fluid flows -- this is called \blue{\it pore velocity}, $\underline{v}$. 
       \item <3-> $\underline{u}$ and $\underline{v}$ are related as,
          \visible<3->{\begin{displaymath}
             \underline{u} = \phi \underline{v}
          \end{displaymath} 
          where $\phi$ is the rock porosity.}
       \item <4-> A few definitions:
          \begin{enumerate}[(a)]\scriptsize
             \item <4-> (Absolute) Porosity $\left(\phi\right)$ is the ratio between total volume of voidage in the rock pore and the volume of the rock,
                \begin{equation}
