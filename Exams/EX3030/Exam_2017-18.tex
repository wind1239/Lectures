...
\begin{question}
  ...
  \item In a counterflow double-pipe heat exchanger, water $\left(C_{p}= 4.18\text{ kJ.kg}^{-1}.^{\circ}\text{C}\right)$ at 35$^{\circ}$C is heated by oil $\left(C_{p}= 1.9\text{ kJ.kg}^{-1}.^{\circ}\text{C}\right)$. The mass flow rate of the water stream is 40 kg.min$^{-1}$ and 170.97 kg.min$^{-1}$ of oil enters the heat exchanger at 110$^{\circ}$C. The overall heat-transfer coefficient is 320 W.$\left(\text{m}^{2}.^{\circ}\text{C}\right)^{-1}$. Calculate:
    \begin{enumerate}[(a)]
      \item Exit water and oil temperatures;\marks{9}
          \solution{The energy balance for both fluid streams (oil and water), where $T_{oil,in}=T_{h,in}=110^{\circ}$C and $T_{w,in}=T_{c,in}=35^{\circ}$C, ~\solmarks{2}
            \begin{eqnarray}
              &&Q_{w} + Q_{oil} = 0 \nonumber \\
            ...
            \end{eqnarray}
            $T_{c,out}=T_{w,out}$ can be obtain as a function of $T_{h,out}$. However, with $Q=U A \Delta T_{lm}$,~\solmarks{1}
            \begin{displaymath}
            ...
            \end{displaymath}
            and substituting $T_{c,out}$ in this expression we obtain $T_{h,out}=79.70^{\circ}$C\solmarks{3}. Now,\solmarks{3}
            \begin{displaymath}
        ...
        \item Total heat transfer (in kW);\marks{2}
          \solution{ The heat transferred between streams can be obtained from~\solmarks{2}
            \begin{displaymath}
...
\end{displaymath}
    ...
    \end{displaymath}
    \solution{In a 1D plane wall divided in $M$ nodes with an internal node $N$. Spatial increment is given by $\Delta x$. The expansion in Taylor series around node $N$ truncating in the second derivative leads to:\solmarks{3}
      \begin{displaymath}
        \begin{cases}
        ...
        \end{cases}
      \end{displaymath}
      Summing these expressions leads to\solmarks{1}
\begin{displaymath}
  \frac{T_{n-1}-2T_{n}+T_{n+1}}{\Delta x^{2}} = \left.\frac{\partial^{2} T}{\partial x^{2}}\right|_{N}.
\end{displaymath}
    }
...
%=============
\solution{ The thermal diffusivity,~\solmarks{1}
\begin{displaymath}
...
\end{displaymath}
and the Fourier number,~\solmarks{1}
\begin{displaymath}
...
The discretised thermal equation,~\solmarks{2}
\begin{eqnarray}
...
Thus for $j=0$:%~\solmarks{2/10}
         ...
         \end{eqnarray} 
         with ghost-cell, $T_{5}^{0}$ defined through the Newmann BC:~\solmarks{2}
         \begin{displaymath}
