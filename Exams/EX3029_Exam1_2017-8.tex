  ...
  \begin{enumerate}[a)]
    \item Figure~\ref{Figure:Fig1} shows a $P-xy$ phase diagram for an arbitrary binary mixture that is vaporised at constant temperature (M-S).
      \begin{figure}[h]
        ...
        \end{center}
      \end{figure}
      Determine (i)-(ix) at Table~\ref{Table:Tab1},\marks{9}
      \begin{table}[h]
        \begin{center}
      ...
      \solution{
        \begin{enumerate}[(i)]
           \item $x_{1}=x_{z}$;\solmarks{1/9}
           \item $y_{1}=y_{BP}$;\solmarks{1/9}
           \item vapour-liquid mixture;\solmarks{1/9}
           \item $x_{1}=x_{Q}$;\solmarks{1/9}
           \item $y_{1}=y_{Q}$;\solmarks{1/9}
           \item dew point $\left(P_{DP}\right)$;\solmarks{1/9}
           \item $x_{1}=x_{DP}$;\solmarks{1/9}
           \item $y_{1}=y_{z}$;\solmarks{1/9}
           \item $x_{1}=0$;\solmarks{1/9}
        \end{enumerate}
        }
        
%====================================================
%%% Moran & Shapiro (Example 11.5)      
     \item Using the Redlich-Kwong equation of state, develop algebraic expressions for the changes in specific entropy and internal energy of a gas between two states where the temperature is the same, $T_{1}=T_{2}$, and the pressures are $P_{1}$ and $P_{2}$, respectively.\marks{11}
%====================
