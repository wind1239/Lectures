...
\begin{question}
     In the design of a new process, nitrogen dioxide $\left(\text{NO}_{2}\right)$ is produced through the decomposition of nitrogen tetroxide $\left(\text{N}_{2}\text{O}_{4}\right)$,
        ...
        \begin{enumerate}[A.]
           \item Decomposition of N$_{2}$O$_{4}$ at 25$^{\circ}$C;
           \item Initial dilution with inert N$_{2}$ prior to the decomposition of N$_{2}$O$_{4}$ at 25$^{\circ}$C;
           \item Decomposition of N$_{2}$O$_{4}$ at 126.85$^{\circ}$C.
        \end{enumerate}
...
\end{eqnarray}
Solving this integral for $T=400$ K leads to $K_{400\text{ K}} = 51.4157$~\solmarks{1/8}, with
         \begin{displaymath}
            y_{N_{2}O_{4}} = \frc{1-\varepsilon}{1+\varepsilon}\;\;\;\;\;\text{ and }\;\;\;\;\;\; y_{NO_{2}} = \frc{2\varepsilon}{1+\varepsilon},
         \end{displaymath}
         and
         ...
         \begin{displaymath}
             K_{400\text{ K}}   = 51.4157 \hspace{1.2cm} \Longrightarrow \varepsilon = 0.9632  \hspace{.35cm} \Longrightarrow \;\;  y_{NO_{2}} = 0.9813 \text{ and } y_{N_{2}O_{4}} = 0.0187
         \end{displaymath}
         
        Note that at such high temperature (400 K) the forward reaction becomes dominant with nearly 98.1$\%$ of NO$_{2}$ being produced.
           }
         \item Which scenario will lead to larger NO$_{2}$ production? Why?\marks{2}
           \solution{At room temperature conditions (A and B), the equilibrium constant is 0.1484 with reaction coordinate ranging from 0.0715 (B) to 0.1891 (A). At 400 K (C), equilibrium constant reaches 51.4331 $\left(\text{with }\varepsilon=0.9632\right)$, \ie, nearly all N$_{2}$O$_{4}$ is decomposed to produce NO$_{2}$. Thus, the best scenario is C.~\solmarks{2/2}

             }          
     \end{enumerate}
      Given:
         \begin{displaymath}
           \Delta M_{r}^{\circ} = \summation[\nu_{i}\Delta M_{f,i}^{\circ}]{i=1}{\mathcal{C}},%  \hspace{1cm} \text{with } M=\left\{U,H,G,S,A\right\}, 
         \end{displaymath}
  ...
  \begin{enumerate}[a)]
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       \solution{The generic form of cubic equations of state is defined as,~\solmarks{1/12}
              ...
              \begin{displaymath}
                 \beta = 5.6674\times 10^{-2}\;\;\text{ and }\;\; q = 3.0413
              \end{displaymath}
              We can simplify the generic form of cubic EOS as,
              \begin{displaymath}
                  Z =  1 + \beta - q\beta\frc{Z-\beta}{Z^{2}}
              \end{displaymath}
              And solving it (calculator), we can obtain Z = 0.8718 \solmarks{3/12} (using Z=1 as initial guess). Molar volume can be obtained through \solmarks{3/12} 
              \begin{displaymath}
                  V = \frc{Z R T}{P} = 5.7492\times 10^{-4}\;\text{m}^{3}.\text{mol}^{-1}.
              \end{displaymath}
              \medskip
              \begin{center} {\bf OR}\end{center}
              Calculating $a$ and $b$\solmarks{6/12}
              \begin{eqnarray}
                a = \frc{27}{64}\frc{R^{2}T_{c}^{2}}{P_{c}}&=&4.2527\times 10^{-6} \text{ m}^{6}\text{bar mol}^{-2}, \nonumber \\
                b = \frc{1}{8}\frc{R T_{c}}{P_{c}} &=&3.7373 \times 10^{-5} \text{m}^{3}\text{mol}^{-1}, \nonumber
              \end{eqnarray}
              and plug them in
              \begin{displaymath}
                P=\frc{R T}{V-b} - \frc{a}{V^{2}}
              \end{displaymath}
              leads to (calculator) {\bf 5.7492$\times$10$^{-4}$ m$^{3}$mol$^{-1}$}\solmarks{3/12} $\left(\text{using V}^{\text{ig}}\text{=6.5944}\times\text{10}^{-4}\text{m}^{3}\text{mol}^{-1}\text{ as initial guess}\right)$. Compressibility factor can be obtained from\solmarks{3/12}
              \begin{displaymath}
                Z=\frc{P V}{R T}=0.8718
              \end{displaymath}
       }
...
%====================
       ...
       \item Using the cyclic rule definition
        \begin{displaymath}
            \left(\frc{\partial P}{\partial T}\right)_{V}\left(\frc{\partial V}{\partial P}\right)_{T}\left(\frc{\partial T}{\partial V}\right)_{P} = -1,
        \end{displaymath}
        obtain an algebraic expression for $\left(\frc{\partial V}{\partial T}\right)_{P}$ for the van der Waals equation of state.\marks{5}
%====================
     ...
     \begin{displaymath}
         \Partial[V]{T}{P} = -\frc{\Partial[P]{T}{V}}{\Partial[P]{V}{T}} = -\frc{\frc{R}{V-b}}{-\frc{RT}{\left(V-b\right)^{2}}+\frc{2a}{V^{3}}}=-\frc{RV^{3}\left(V-b\right)}{2a\left(V-b\right)^{2}-RTV^{3}}
     \end{displaymath} 
...
%%% Johannes T0602
%%%
\item\label{T0602} What is the change in entropy when 0.7 m$^{3}$ of CO$_{2}$ and 0.3 m$^{3}$ of N$_{2}$ form a gas mixture at 1 bar and 25$^{\circ}$C? Assume ideal gases, and given
\begin{displaymath}
\Delta S = - nR\sum\limits_{i=1}^{n}y_{i}\ln{y_{i}},
\end{displaymath}
where $S$, $n$ and $y$ are entropy, number of moles and mole fraction, respectively. $R$ is the molar gas constant.~\marks{10}
...
\begin{eqnarray}
y_{1} = 0.7 \;\;\Longrightarrow V_{1}^{t} = 0.7\;m^{3} \nonumber \\
y_{2} = 0.3 \;\;\Longrightarrow V_{2}^{t} = 0.3\;m^{3} \nonumber
\end{eqnarray}
...
%========================
\solution{ At 331 K, the saturation pressures are $P_{1}^{sat}=$ 42.90 kPa and $P_{2}^{sat}=$ 70.54 kPa~\solmarks{2/10}. The liquid solution with $x_{1}=0.412$ and $x_{2}=0.588$ results in the following activity coefficient $\gamma_{1}=2.0112$ and $\gamma_{2}=1.6702$.~\solmarks{2/10}

The partial pressure of ethanol and n-hexane are,~\solmarks{2/10}
\begin{eqnarray}
P_{1} = x_{1}\gamma_{1}P_{1}^{sat} = 35.55\text{ kPa} \nonumber \\
P_{2} = x_{2}\gamma_{2}P_{2}^{sat} = 69.28\text{ kPa} \nonumber 
\end{eqnarray}
The bubble pressure is~\solmarks{2/10}
\begin{displaymath}
P =P_{1} + P_{2} = 104.83\text{ kPa}
\end{displaymath}
And the composition of the vapour phase is~\solmarks{2/10}
\begin{displaymath}
y_{1} = \frc{P_{1}}{P} = 0.3391\;\;\text{ and } \;\; y_{2} = 0.6609.
\end{displaymath}
...
%%% Moran & Shapiro (Example 11.5)      
     \item Using the Redlich-Kwong equation of state, develop algebraic expressions for changes in specific entropy $\left(s_{2}-s_{1}\right)$ and internal energy $\left(u_{2}-u_{1}\right)$ of a gas between two states at the same temperature $\left(\text{\ie T}_{1}=\text{T}_{2}\right)$ and pressures $P_{1}$ and $P_{2}$.\marks{10}
%====================
...
\end{displaymath} 
determine $T$ and $P$ for the separator. Activity coefficients for the liquid phase are given by,
\begin{displaymath}
