...
%====================
\solution{Solving the cubic polynomial in $Z$,
\begin{displaymath}
Z^{3} -(1+B)Z^{2} +AZ -AB = 0 \Longrightarrow  Z = 0.9436 \nonumber
\end{displaymath}
with $B=bP/(RT)= 2.3281\times 10^{-2}$ and $A=aP/(RT)^{2}= 7.7095\times 10^{-2}$~\solmarks{2/13}, leads to $Z = 0.9436$~\solmarks{3/13}. Now for the fugacity equation, either
\begin{eqnarray}
&& \ln{\left(\frc{f}{P}\right)} = -\ln{\left(1-\frc{b}{V}\right)} - \frc{a}{RTV} - \ln{Z} +\left(Z-1\right) \nonumber \\
&& \text{or} \nonumber \\
&&  \ln{\left(\frc{f}{P}\right)} = -\ln{\left(1-\frc{B}{Z}\right)} - \frc{A}{Z} - \ln{Z} +\left(Z-1\right) \nonumber
\end{eqnarray}
leads to $f =\; 1.3250\times 10^{6}\text{ Pa} = 1.3250\text{ MPa}$.~\solmarks{8/13}. If the first equation was used to calculate $f$, then one needs to calculate the molar volume of the gaseous CO$_{2}$ through
\begin{displaymath}
  V = \frc{ZRT}{P} = 1.7371\times 10^{-3} \text{ m}^{3}.\text{mol}^{-1}
  \end{displaymath}
...
%======================
         \solution{The reaction $N_{2}O_{4}\text{(g)} \Leftrightarrow 2 NO_{2}\text{(g)}$ may be obtained by combining the linearly independent chemical reactions $(1)$ and $(2)$, i.e.,~\solmarks{2/8}
                \begin{displaymath}
...
%======================
         \solution{ The equilibrium constant can also be obtained as a function of the species' activities,~\solmarks{2/12}
             \begin{displaymath} 
                 K = \frc{a^{2}_{NO_{2}}}{a_{N_{2}O_{4}}} = \frc{\left(\frc{\overline{f}_{NO_{2}}}{f^{\circ}_{NO_{2}}}\right)^{2}}{\left(\frc{\overline{f}_{N_{2}O_{4}}}{f^{\circ}_{N_{2}O_{4}}}\right)}
             \end{displaymath}
             Assuming ideal gas behaviour, $\overline{f}_{i}=P_{i}$, $f^{\circ}_{i}=P^{\circ}_{NO_{2}}=P^{\circ}_{N_{2}O_{4}} =$ 1 atm,~\solmarks{2/12}
             \begin{displaymath} 
             ...
             \end{displaymath}
             with $n_{N_{2}O_{4},0}=1$, $n_{NO_{2},0}=0$, $\nu_{N_{2}O_{4}}=-1$, $\nu_{NO_{2}}=2$ and $\nu = 1$~\solmarks{1/12}. Thus, the compositions in equilibrium are~\solmarks{2/12}
              \begin{displaymath}
                  y_{N_{2}O_{4}} = \frc{1-\varepsilon}{1+\varepsilon} \hspace{2cm}\text{ and }\hspace{2cm} y_{NO_{2}}= \frc{2\varepsilon}{1+\varepsilon}
              \end{displaymath}
              Here any initial conditions are acceptable (i.e., any initial number of moles for both species), however this would change the expressions for $y_{N_{2}O_{4}}$ and $y_{NO_{2}}$. Replacing these expressions in the previous equation with $K_{\text{eq},298}= 0.1641$ (from (a))~\solmarks{2/12}
              \begin{displaymath}
...
%
\solution{Assuming that at 1 bar the vapour phase behaves as an ideal gas. The vapour phases fugacities are then equal to the partial pressures. Assume the Lewis-Randall rule applies to the concentrated species $2$ and that Henry's law applies to the diluted species $1$, therefore,
\begin{displaymath}
y_{1}P=\mathcal{H}_{1}x_{1};\;\;\text{ and }\;\; y_{2}P= x_{2}P_{2}^{\text{sat}}
\end{displaymath}
with $x_{1}+x_{2}=1$ and $y_{1}+y_{2}=1$. As $P = \sum\limits_{i=1}^{2}y_{i}P = y_{1}P + y_{2}P$,~\solmarks{5/10}
\begin{displaymath}
P = \mathcal{H}_{1}x_{1}+\left(1-x_{1}\right)P_{2}^{\text{sat}} \;\;\Longrightarrow {\bf x_{1} = 4.5023\times 10^{-3}}
\end{displaymath}
and ${\bf y_{1}=\frc{\mathcal{H}_{1}x_{1}}{P}=0.9005}$.~\solmarks{5/10}
}
%
% SM&VN 10.19 
...
%
\solution{With $x_{A}$ =0.65 and $x_{B}$=0.35, we can calculate the activity coefficients, $\mathbf{\gamma_{A}=1.2467}$ and $\mathbf{\gamma_{B}=2.1393}$~\solmarks{2/10}, and apply in~\solmarks{3/10}
     \begin{displaymath}
         P = x_{A}\gamma_{A} P_{A}^{\text{sat}} + x_{B}\gamma_{B} P_{B}^{\text{sat}} =  \mathbf{1.6712} \text{ {\bf bar}}.
     \end{displaymath}
