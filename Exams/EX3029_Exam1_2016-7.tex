...
\end{eqnarray}
leads to $f =\; 1.3250\times 10^{6}\text{ Pa} = 1.3250\text{ MPa}$.~\solmarks{8/13}
...
%======================
         \solution{ The equilibrium constant can also be obtained as a function of the species' activities,~\solmarks{2/12}
             ...
             with $n_{N_{2}O_{4},0}=1$, $n_{NO_{2},0}=0$, $\nu_{N_{2}O_{4}}=-1$, $\nu_{NO_{2}}=2$ and $\nu = 1$~\solmarks{1/12}. Thus, the compositions in equilibrium are~\solmarks{2/12}
              ...
              \end{displaymath}
              Replacing these expressions in the previous equation with $K_{\text{eq},298}= 0.1641$ (from (a))~\solmarks{2/12}
              \begin{displaymath}
...
%======================
         \solution{ For a binary mixture:~\solmarks{2/12}
             ...
             with~\solmarks{5/12}
             \begin{displaymath}
                 \frc{G^{E}}{R T} = x_{1}x_{2} \left(A + B x_{1}^{2}\right)\;\Longrightarrow\; \frc{\d \left(G^{E}/RT\right)}{\d x_{1}} = \left(1-2x_{1}\right)A + \left(3-4x_{1}\right)Bx_{1}^{2}
             \end{displaymath}
             ...
             \end{eqnarray}
             Alternatively, one could also further develop/simplify the equations above considering $x_{1}=1-x_{2}$.
...
\end{displaymath}
with $x_{1}+x_{2}=1$. Thus $P = \sum\limits_{i=1}^{2}y_{i}P = y_{1}P + y_{2}P$ becomes,~\solmarks{5/10}
\begin{displaymath}
