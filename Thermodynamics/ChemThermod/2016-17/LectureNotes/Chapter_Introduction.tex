...
%%%
   ...
   The physical condition (\ie state) of a substance is defined by its physical properties. For example, the state of a pure gas is specified by volume ($V$), mass (through the number of moles, $n$), pressure ($P$) and temperature ($T$).

   Laboratory experiments on fluids revealed that if three of these properties (\eg $V, n, T$) are specified, the state of the fluid is naturally defined, and the forth property is fixed. Mathematically, this can be represented by the functional often referred as {\bf equation of state}\index{Equation of State} (EOS),
   \begin{displaymath}
     P = f(T,V,n).
   \end{displaymath}
