...
%%% Subsection
   ...
   {\it Cycles} are defined as any process (or a set of processes) in which the end states are identical  Any process or series of processes whose end states are identical is termed a cycle. For example, in Fig.~\ref{Chapter:Introduction:Fig:CyclesSchematic} a closed system, initially at state 1 $\left(\text{with coordinates } P_{1},V_{1}\right)$, is driven (by simultaneous changes in pressure, volume and temperature) to the state 2 through pathway $A$. After a while, the system is restored to the original state $1$ through pathway $B$. These two processes, $A$ and $B$, constinute a cycle as the final state is the same as the initial state.
