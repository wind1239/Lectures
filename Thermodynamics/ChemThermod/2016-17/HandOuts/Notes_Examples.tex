...
\bigskip
    ...
    \begin{shaded}
       ...
       \end{equation}
       Assuming  that at $t=0$, the initial composition is $n_{i} = n_{i,0}$ and the reaction has not started  yet, \ie $\varepsilon=0$. Integrating Eqn.~\ref{Mod6:Eqn:2a} from $t=0$ to $t_{i}$,
       \begin{eqnarray}
...
\begin{subequations}
   Given the homogeneous (\ie in a single-phase system) chemical reaction
...
\begin{subequations}
   ...
   For simplicity of the analysis, let's assume {\it single homogeneous chemical reaction}. The total Gibbs free energy of a system is obtained from
     \begin{equation}
         G^{t} = \summation[n_{i}G_{i}]{i=1}{\mathcal{C}} = \summation[\left(n_{i,0}+\nu_{i}\varepsilon\right)G_{i}]{i=1}{\mathcal{C}}\label{Mod6:Eqn:4a}
     \end{equation}
   where $G_{i}$ is the molar Gibbs free energy. At equilibrium (Fig.~\ref{Mod06Fig02}), $\left(dG^{t}\right)_{T,P} = 0$, \ie
       \begin{eqnarray}
          && dG^{t} = d(nG) = ndG + Gdn \;\;\;\; \blue{\times \frc{1}{d\varepsilon}} \nonumber \\
          && \frc{dG^{t}}{d\varepsilon} = n\frc{dG}{d\varepsilon} + G\frc{G dn}{d\varepsilon} = 0 \nonumber \\
          && \text{in a closed system, } dn = 0 \Longrightarrow \frc{dG^{t}}{d\varepsilon} = \frc{dG}{d\varepsilon} = 0\;\;\text{ (equilibrium criteria).} \nonumber
       \end{eqnarray}
      \begin{figure}[hpt]
         \begin{center}
...
\subsubsection{Van't Hoff Equation}
\begin{subequations}
   ...
   where $\overline{f}_{i}$ is the fugacity of component $i$ in the mixture\footnote{In Module~\ref{Section:05}, we have studied forms to calculate $\mfr[\overline{f}]{i}{j}$:
      ...
      \end{enumerate}
}. At {\it standard state} (\ie pure component at 25$^{\circ}$C and 1 atm), the Gibbs free energy of formation of species $i$ is 
      \begin{displaymath}
            G_{i}^{\circ} = RT\ln{f_{i}^{\circ}} + C_{i}(T)
      \end{displaymath}
      Subtracting the equation above from the previous one,
      \begin{equation}
         \mu_{i} - G_{i}^{\circ} = RT\ln{\frc{\overline{f}_{i}}{f_{i}^{\circ}}}.\label{Mod6:Eqn:5a}
      \end{equation}
      Now, if Eqn.~\ref{Mod6:Eqn:5a} is substituted in Eqn.~\ref{Mod6:Eqn:4b}, eliminating $\mu_{i}$\footnote{Here {\it log} properties (see Appendix C) are used to obtain the product $\prod$,
         \begin{eqnarray}
            \summation[\ln{\left(\frc{a_{i}}{b_{i}}\right)^{n_{i}}}]{i=1}{\mathcal{Z}} &=& \ln{\left(\frc{a_{1}}{b_{1}}\right)^{n_{1}}} + \ln{\left(\frc{a_{2}}{b_{2}}\right)^{n_{2}}}  + \ln{\left(\frc{a_{3}}{b_{3}}\right)^{n_{3}}} + \cdots + \ln{\left(\frc{a_{\mathcal{Z}}}{b_{\mathcal{Z}}}\right)^{n_{\mathcal{Z}}}} \nonumber \\
      ...
      \begin{eqnarray}
         && \summation[\nu_{i}\mu_{i}]{i}{} = \summation[\nu_{i}\left(G_{i}^{\circ} + RT\ln{\frc{\overline{f}_{i}}{f_{i}^{\circ}}}\right)]{i}{} = 0 \;\;\;\Longleftrightarrow \;\;\; \summation[\nu_{i}G_{i}^{\circ}]{i}{} + RT\summation[\ln{\left(\frc{\overline{f}_{i}}{f_{i}^{\circ}}\right)^{\nu_{i}}}]{i}{} = 0,  \nonumber \\
         && \ln{\left[\prod\limits_{i=1}^{\mathcal{C}} \left(\frc{\overline{f}_{i}}{f_{i}^{\circ}}\right)^{\nu_{i}}\right]} = -\frc{\summation[\nu_{i}G_{i}^{\circ}]{i}{}}{RT} \;\;\;\Longleftrightarrow \;\;\; \prod\limits_{i=1}^{\mathcal{C}} \left(\frc{\overline{f}_{i}}{f_{i}^{\circ}}\right)^{\nu_{i}} = \exp{\left[-\frc{\summation[\nu_{i}G_{i}^{\circ}]{i}{}}{RT}\right]}\label{Mod6:Eqn:5b}
      \end{eqnarray}
      \begin{shaded}
         We have defined the standard state Gibbs energy of reaction in Eqn.~\ref{Mod6:Eqn:3b},
         \begin{displaymath}
           \Delta G^{\circ}_{\text{r}} = \summation[\nu_{i} G^{\circ}_{i,f}]{i}{}\;\;\;\;\text{ and defining }\;\;\;\;\Delta G^{\circ} \equiv \summation[\Delta G^{\circ}_{\text{r}}]{i}{},
         \end{displaymath}
         as the {\it standard Gibbs free energy change of reaction}, \ie the Gibbs free energy change of reaction with each species (reactants and products) in its standard state (pure species at 1 atm) and at system temperature. Now, defining the \blue{\it equilibrium constant}, \blue{$K$},
         \begin{displaymath}
            K = \exp{\left[-\frc{\Delta G^{\circ}}{RT}\right]}
         \end{displaymath}
         and replacing in Eqn.~\ref{Mod6:Eqn:5b},
         \begin{equation}
            K = \prod_{i=1}^{\mathcal{C}} \left(\frc{\overline{f}_{i}}{f_{i}^{\circ}}\right)^{\nu_{i}} = \prod_{i=1}^{\mathcal{C}} a_{i}^{\nu_{i}},\label{Mod6:Eqn:5c}
         \end{equation}
         where $a_{i}$ is the activity of species $i$ in the reaction mixture. The equilibrium constant $K$ depends on the system temperature, \ie $K=K(T)$, and therefore is important to be able to define it at any conditions. The {\it Van't Hoff equation} provides a critically important way to correlate $K$ and the enthalpy of reaction. Using Eqn.~\ref{Mod03_GibbsGeneratingFunctionPConst} (for constant pressure conditions) at standard state,
         \begin{displaymath}
              H^{\circ}_{i} = -RT^{2} \frc{d}{dT}\left(G_{i}^{\circ}/RT\right) \;\;\;\;\Longrightarrow \;\;\;\; \Delta H^{\circ} = -RT^{2} \frc{d}{dT}\left(\Delta G^{\circ}/RT\right),\label{Mod6:Eqn:5d}
         \end{displaymath}
         where $\Delta H^{\circ}$ is the standard heat (or enthalpy) of reaction. Replacing $\Delta G^{\circ}/RT=-\ln{K}$ in the equation above leads to the \blue{Van't Hoff equation}
         \begin{equation}
             \frc{d\left(\ln{K}\right)}{dT} = \frc{\Delta H^{\circ}}{RT^{2}}\;\;\;\text { or }\;\;\;\;  \frc{d}{dT}\left(\Delta G^{\circ}/RT\right) = -\frc{d\left(\ln{K}\right)}{dT} = -\frc{\Delta H^{\circ}}{RT^{2}}   \label{Mod6:Eqn:5e}
         \end{equation}
      \end{shaded}
