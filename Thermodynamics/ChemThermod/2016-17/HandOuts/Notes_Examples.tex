...
This is mainly due to the fact that EOS were originally (and cubic EOS in particular) to represent weak inter-molecular forces between molecules at relatively large distances. As pressure increases $\left(P\rightarrow \infty\right)$ and the distance between molecules tends to zero $\left(d\rightarrow 0\right)$, accuracy of EOS to predict $PVT$ behaviour also decreases, as the attractive and repulsive forces between molecules are stronger. 

Molecules in liquid phase are held together at close distance by strong attractive forces, however these forces are not strong enough to keep them in a fixed position (as forces in the solid phase) and the molecules are free to move past, slide over and collide with other molecules. A fraction of the molecules (mostly with higher energy) may be able to break such attractive forces and `escape' towards the vapour phase. 

Kinetic energy in gaseous molecules are larger than the attractive forces, leading to large distances between molecules and a tendency to occupy the whole volume of the system. The large kinetic energy in gaseous molecules also lead to continuous collisions between molecules and with the system (container) walls. After collisions, although the momentum is conserved, individual energies (and velocities) may decrease and molecules may move to a system with lower kinetic energy, \ie liquid phase.
...
\begin{subequations}
  ...
  \end{displaymath}
  Using the Maxwell relation, Eqn.~\ref{Mod03_MaxwellRelation7},
  \begin{eqnarray}
  ...
  \begin{displaymath}
     \Partial[\mu^{\ast}]{P}{T} = \frc{RT}{P},
  \end{displaymath}
  where the superscript $^{\ast}$ refers to ideal gas. This differential can be integrated as,
  \begin{equation}
    \mu^{\ast} = RT\ln{P} + C(T),\label{Mod05_fugacity1a}
  \end{equation}
  ...
  \end{equation}
  where $f$ is the {\it fugacity of a real gas}, \ie \blue{a representation of pressure of an ideal gas which has the same chemical potential as the real gas.} Since the {\it fugacity} has the \underline{units of pressure}, it is often referred as a `fictitious pressure'.  The definition of fugacity from Eqn.~\ref{Mod05_fugacity1b} is entirely general and can be readily extended to both liquids and solids. Now, replacing Eqn.~\ref{Mod05_fugacity1b} in the Maxwell relation, Eqn.~\ref{Mod03_MaxwellRelation7},
  \begin{equation}
    RT\Partial[\left(\ln{f}\right)]{P}{T} = \overline{v},\label{Mod05_fugacity1c}
  \end{equation}
      {\it fugacity} can be obtained by integrating this equation holding $T$ constant. As pressure tends to zero (lower limit of the left-hand-side integration), the fluid behaves as an ideal gas. We can thus define that the fugacity of a pure component is equal to the pressure in the limit of zero pressure, \ie
      \begin{shaded}
        \begin{equation}
      ...
      In a \underline{mixture}, Eqn.~\ref{Mod05_fugacity1c} becomes,
      \begin{eqnarray}
...
%%% SUBSECTION
\subsection{Activity}\label{Section:05:ActivitySection}
   \begin{subequations}
        
      \begin{shaded}
         An \underline{ideal solution} is a mixture in which,
        \begin{equation}
          \mfr[\overline{f}]{i}{V} = y_{i}P'\;\;\;\text{ and }\;\;\; \mfr[\overline{f}]{i}{L} = x_{i}\mfr[f]{i}{L}\label{Mod05_fugacity1g}
        \end{equation}
