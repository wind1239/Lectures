...
In the previous modules, we mostly focused on thermodynamic properties of systems containing pure chemical species. This module (and the remaining of the course) will study system with arbitrary number of components, and the quantification of each component at each phase. A few composition definitions:
\begin{subequations}
   \begin{enumerate}[a)]
       \item Molar (or mole) $\left(x_{i}\right)$ fraction:
            \begin{eqnarray}
                x_{i} = \frc{n_{i}}{n}.
            \end{eqnarray}
            where $n$ and $n_{i}$ are the total number of moles in the system and the number of moles of component $i$, respectively. As these quantities are normalised, we can define:
            \begin{equation}
                  \summation[x_{i}]{i=1}{\mathcal{N}} = 1
            \end{equation}
            where $\mathcal{N}$ is the total number of components in the mixture;
...
{\it Partial molar properties} describes the behaviour of homogeneous multi-component systems. Let's consider a homogeneous (\ie single phase) and open system (\ie system that allows mass and energy transfer across the boundary towards the surroundings) that undertakes a change in composition. Thus, the total value of any extensive property $M^{\text{t}}\;\left(M\equiv V, U, H, S, G, A\right)$ is not only a function of pressure and temperature, but it also depends on the number of moles of each species in the system. therefore
  \begin{subequations}
     ...
     \end{equation}
     where $n=\summation[n_{i}]{i=1}{\mathcal{N}}$ is the total number of moles of all species. The total derivative of Eqn.~\ref{Mod04_PartialProperties1} is,
     \begin{displaymath}  
...
Now, let's consider a closed system consisting of two phases in equilibrium  (vapour and liquid, or solid and vapour or liquid and solid), $\alpha$ and $\beta$. Each of these phases may be considered as an open system with an interface between them, where mass and energy can flow freely. Also, both systems (or phases) are in mechanical and thermal equilibrium $\left(\ie\; T^{\alpha}=T^{\beta}=T\text{ and } P^{\alpha}=P^{\beta}=P\right)$. We can apply Eqn.~\ref{Mod04_ChemPotentialDef1c} to both phases,
  \begin{subequations}
     ...
     This equation describes changes in the chemical potential and in the mass of each component in each phase. In fact, throughout this derivation, $dn_{i}^{\alpha}$ is enforced to be independent of each other and therefore \underline{can not be zero}, hence
      ...
      \begin{shaded}
         \begin{equation}
            \blue{\mu_{i}^{\alpha} = \mu_{i}^{\beta} = \cdots = \mu_{i}^{\mathcal{P}} \;\;\;\;\forall i=1,2,\cdots, \mathcal{N}.}\label{Mod04_ChemPotentialDef1c3} 
         \end{equation}
      \end{shaded}
...
Now, let's assume that the fluid of this diagram is at liquid phase at pressure $P_{K}$ $\left(\text{coordinate }\red{K_{M}}\right)$ with composition \blue{$x_{1}$} and \blue{$y_{1}=0$}. As pressure is reduced along the \blue{$K_{M}-Q$} line, the first bubble of vapour appears at \blue{$M$}; composition at this coordinate is \blue{$\left(x_{1},y_{1}\right)$}, and this is called \blue{\it bubble point}. Further reducing the pressure to coordinate \blue{$K_{i}$} (still at constant temperature), the fluid becomes partially vaporised and the composition at this coordinate is given by the {\it tie-line} \blue{$K_{1}-K_{2}$} -- \blue{$\left(x_{1}=\beta,y_{1}=\alpha\right)$}. As the pressure is continuously reduced along line \blue{$K_{i}-N$}, more liquid is vaporised until \blue{$N$}, when the last droplet of liquid (dew) is vaporised. This coordinate is called \blue{\it dew point}. The composition at $N$ is \blue{$\left(x_{1},y_{1}\right)$}. Continued reduction of pressure leads into (pure) vapour region with composition \blue{$\left(x_{1}=0,y_{1}\right)$}.
      \begin{figure}[h]
         \begin{center}
             \includegraphics[width=.75\columnwidth,clip]{./Figs/Pxy_Txy_Digrams}\vspace{-1cm}
             \vspace{-.1cm}\caption{VLE for binary mixtures: (a) $P-xy$ diagram for three temperatures, and (b) $T-xy$ diagram for three pressures (extracted from Smith, Van Ness and Abbott, 2000).}\label{Mod04Fig03}
         \end{center}
      \end{figure}
      $P-T-xy$ diagram, although insightful, is not very practical. Projection of $P-xy$ diagram at three constant temperature planes and $T-xy$ at three constant pressures are shown in Fig.~\ref{Mod04Fig03}(a) and (b) with associated {\it tie-lines}, respectively. Now, let's consider the plane \blue{$M-N-Q-T-S$} ($T$ is in hidden in the back of the diagram at $P=0$ plane) perpendicular to the composition axis. The projection of this plane

      \begin{figure}[h]
         \vbox{\hbox{\hspace{2cm}\includegraphics[width=.75\columnwidth,clip]{./Figs/PTlocus_Digram}}
                \vspace{-2.5cm}\hbox{\hspace{2cm}(a)\hspace{2cm}(b)}}
                \caption{VLE for binary mixtures: (a) $P-T$ diagram for several compositions, and (b) $P-T$ diagram in the critical region (extracted from Smith, Van Ness and Abbott, 2000).}\label{Mod04Fig04}
      \end{figure}
