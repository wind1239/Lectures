   ...
   \subsubsection{Properties obtained at Constant Temperature}
      ...
      For processes at constant temperature, \ie, $dT=0$,
          ...
          \begin{eqnarray}
                 ...
                 \frc{S^{R}}{R} &=& \underbrace{-T\int\limits_{0}^{P}\left.\Partial[Z]{T}{}\right|_{P}\frc{dP}{P}}_{\text{Eqn.~\ref{Mod03_ResidualProperties_GeneratingEnthalpyFunction2}}} \overbrace{- \Phi - \int\limits_{0}^{P}\left(Z-1\right)\frc{dP}{P}}^{\text{Eqn.~\ref{Mod03_ResidualProperties_GeneratingGibbsFunction2}}}\;\;\;\blue{(\text{at constant}\;\; T)}.\label{Mod03_ResidualProperties_GeneratingEntropyFunction1}
          \end{eqnarray}
...
%%% SUBSUBSECTION
   \subsubsection{Residual Properties in the Zero-Pressure Limit}
       ...
       By definition, at pressures near to zero $P\rightarrow 0$, the gas behaves as an ideal gas and $Z\rightarrow 1$. However, \underline{not all} residual properties become zero near the near-zero pressure condition. Molar volume for example,
       \begin{displaymath}
...
both terms in the r.h.s. tends to $+\infty$ and the difference is assumed undetermined. For other properties:
       \begin{eqnarray}
           \lim\limits_{P\rightarrow 0}H^{R} &=& 0, \hspace{2cm} \forall T \text{ (for all $T$)}, \nonumber \\
           \lim\limits_{P\rightarrow 0}\left(G^{R}/RT\right) &=& \infty-\infty, \hspace{1cm}  \text{undetermined but not necessarily zero}. \nonumber 
       \end{eqnarray}
...
We can define the {\it latent heat of phase transition} (\eg vaporisation, solidification etc), $\Delta H^{\alpha\beta}$ from Eqn.~\ref{Mod03_FundamentalRelation02} integrating with constant pressure,
      \begin{displaymath}
          dH = TdS + VdP \;\;\Rightarrow \;\; \int dH = \int TdS \;\;\Rightarrow \;\;  H^{\beta}-H^{\alpha} = T\left(S^{\beta}-S^{\alpha}\right)  \;\;\Rightarrow \;\; \Delta H^{\alpha\beta} = T\Delta S^{\alpha\beta},
      \end{displaymath}
therefore,
      \begin{shaded}
          \begin{equation}
...
if we treat the vapour as an ideal gas, $V^{\text{g}} = \frc{RT}{P}$, and replacing in Eqn.~\ref{Mod03_ClapeyronEqn} 
      ...
      \end{displaymath}
For infinitesimal intervals of $T$, \underline{$\Delta H^{\text{fg}}$ may be considered as a constant}, thus
%
           \begin{figure}[h]
               \begin{center}
...
For a generic thermodynamic potential $M$ (= $V$, $U$, $H$, $S$ etc),
    \begin{shaded}
       \begin{subequations}
           ...
           \end{equation}
           \begin{equation}
              M = \mfr[M]{}{L} + \mfr[x]{}{V}\Delta\mfr[M]{}{LV}
           \end{equation}
       \end{subequations}
    \end{shaded}
$\mfr[x]{}{V}$ is called {\it vapour quality}. Thermodynamic potentials of pure substances are graphically represented by $PH$ (pressure $\times$ enthalpy) and $TS$ (temperature $\times$ entropy) diagrams, Fig.~\ref{Mod03Fig02}, where information on $P$, $T$, $S$, $H$, $x$ and $V$ can be readily extracted.
%
           \begin{figure}[h]
              \vbox{
