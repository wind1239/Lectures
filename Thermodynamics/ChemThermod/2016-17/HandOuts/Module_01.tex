  ...
  \begin{columns}
     ...
     \begin{column}[l]{0.5\linewidth}
        \begin{enumerate}[(a)]\setcounter{enumi}{6}%\scriptsize
           ...
           \item<6-> \blue{Time management} is crucial. \red{Do not leave to study all contents during the \underline{Revision Week}.}   
        \end{enumerate}
     \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}
 \frametitle{Aims and Objectives}
     ...
     \begin{enumerate}[(a)]
      \item We aim to understand how energy evolves or is consumed in industrial- or environmental-based processes.
      \item In order to achieve this, we must revise definitions of:
       \begin{enumerate}[(i)]
        \item Intensive and extensive properties;
        \item Laws of thermodynamics;
        \item Reversible and irreversible processes;
       \end{enumerate}
      \item Finally, we should revise applications of \red{{\it First and Second Laws}}. 
     \end{enumerate}
\end{frame}
 ...
 \frametitle{An `Extremely Short History' of Thermodynamics}
   ...
   {\begin{block}<3->{1871: Scottish mathematical physicist James Clerk Maxwell}
     \begin{itemize}
       ...
       \item \textcolor{blue}{Maxwell's Laws of Eletromagnetism (1861-1881).}
...
\begin{frame}
 \frametitle{The Thermodynamic System}
  \begin{enumerate}[(a)]%\scriptsize
   ...
   \item <4-> It can be applied to systems in equilibrium and non-equilibrium;
   \item <5-> In this course we focus on \blue{system at equilibrium}..     
  \end{enumerate}
  \visible<6->{\begin{block}{\begin{center} Thermodynamics {\bf \underline{does}}:\end{center}}
     ...
     \end{enumerate} 
  \end{block}  }
  \visible<7->{\begin{block}{\begin{center} Thermodynamics {\bf \underline{does not}}:\end{center}}
     \begin{enumerate}[(i)]%\scriptsize
 ...
 \frametitle{System Properties}
  \begin{columns}
    ...
    \begin{column}[l]{0.6\linewidth}
      \begin{enumerate}
         \item <2-> Any characteristic of a system is called a \textcolor{red}{property} -- e.g., pressure, temperature, mass, etc;
         \item <2-> Properties can be classified as \textcolor{red}{intensive} or \textcolor{red}{extensive};
         \item <3-> \textcolor{red}{Intensive properties} are those that are {\bf independent} of the mass of a system -- e.g., temperature, pressure, density, viscosity, etc;
         \item <4-> \textcolor{red}{Extensive properties} are those whose values {\bf depend} on the size (or extent) of the system -- e.g., mass, volume, number of moles, internal energy, enthalpy, entropy, etc.  
      \end{enumerate} 
    \end{column}
  \end{columns}
\end{frame}
...
\vspace{-.8cm}
   \begin{figure}%
     \hbox{\hspace{-.1cm}\includegraphics[width=8.cm,clip]{./../Pics/FirstLaw_Cycle_02}}
   \end{figure}    
  \end{column}
 \end{columns}
 \normalsize
...
\begin{frame}
 \frametitle{Representations of the First Law -- Process}
   ...
   \end{equation}
   \item <3-> B and C are arbitrary paths ; Eqn. \ref{Module00:proc1c} asserts that the integral of $\left(\delta Q -\delta W\right)\left.\right|_{2}^{1}$ is \underline{path-independent}. Notice, however that both \textcolor{blue}{Q} and \textcolor{blue}{W} are \underline{path-dependent} quantities. 
    \end{itemize}
\end{frame}
...
\begin{frame}
 \frametitle{Reversible Processes}
   \begin{itemize}
    \item <1->The initial state $\left(\text{i.e., }T_{0}\right)$ can be restored {\it but not spontaneously} $\Longrightarrow$  The reverse processes do not violate the First Law, and yet they {\it do not occur spontaneously};
    \item <1->Another law is needed to help understand and predict which processes will occur spontaneously.
   \end{itemize}
  
   \visible<2->{\begin{block}{Reversible process}
     A process in which it is possible to return both the system and surroundings to their original states;
   \end{block}}
   \visible<3->{\begin{block}{Irreversible process}
      A process in which it is impossible to return both the system and surroundings to their original states.
   \end{block}}
 \normalsize
\end{frame}
...
%
     \visible<1->{\begin{block}{Equation of State}
           \visible<1->{\begin{displaymath}
              P V = R T
           \end{displaymath}} 
     ...
     \end{block}}
%
     \visible<2->{\begin{block}{Heat Capacities:}
           \visible<2->{\begin{eqnarray}
           ...
           \end{eqnarray}
        Internal energy {\bf depends only on} the temperature, $U = U(T)$.}
     \end{block}}
\end{frame}
...
%
     ...
     \end{block}}
%
     \visible<2->{\begin{block}{Relationship between $C_{p}(T)$ and $C_{v}(T)$:}
           \visible<2->{\begin{displaymath}
...
%
     \visible<1->{\begin{block}{Processes involving {\it heat} and {\it work} quantities:}
     ...
     \end{block}}  
%
     \visible<2->{\begin{block}{With $PV = RT$:}
           \visible<2->{\begin{displaymath}
              d W = -P d V  \Longleftrightarrow dQ = \frac{C_{V}}{R}VdP + \frac{C_{P}}{R}P d V  
           \end{displaymath}}    
     \end{block}}  
%
     \visible<3->{\begin{block}{Isothermal procesess $(dT = 0)$:}
           \visible<3->{\begin{displaymath}
...
\begin{frame}
 \frametitle{Ideal Gas: Processes}
%
     \visible<1->{\begin{block}{Isobaric process $(dP= 0)$:}
           \visible<1->{\begin{displaymath}
              Q = \Delta H = \int\limits_{T_{1}}^{T_{2}}C_{P}dT
           \end{displaymath}}    
     \end{block}}  
%
     \visible<2->{\begin{block}{Isochoric process $(dV= 0)$:}
           \visible<2->{\begin{displaymath}
...
\begin{frame}
 \frametitle{Ideal Gas: Processes}
%
     \begin{block}{Adiabatic process $(dQ= 0)$:}
           \begin{displaymath}
...
\begin{frame}
 \frametitle{Summary}
After this Module you should know the main fundamentals and definitions of classic thermodynamics which will be relevant to understand the remaining of the course. This includes knowledge of: 
 \begin{enumerate}[(a)]
  \item <2-> Zeroth, First and Second laws;
  \item <2-> Reversibility and its impact on thermodynamics formulation;
  \item <2-> Definition of the key thermodynamic properties, e.g., work, heat, temperature, internal energy, enthalpy and entropy;
  \item <2-> Main thermodynamic relations derived from the First Law.
 \end{enumerate}
\end{frame}
...
\begin{frame}
 \frametitle{Example 1}
         \blue{If $P_{1}$ = 3.00 atm, $V_{1}$ = 500 cm$^{3}$, $P_{2}$ = 1.00 atm and $V_{2}$ = 2000 cm$^{3}$. Calculate the work, $W_{\text{rev}}$ (in $J$), for the expansion processes shown in the Figures (a) and (b).}
      \begin{figure}[h]
         \begin{center}
           \includegraphics[width=.6\columnwidth,clip]{./Figs/Mod1Ex1}
           \vspace{-.1cm}\caption{\blue{Expansion processes.}}
         \end{center}
       \end{figure}
\end{frame}
...
\begin{frame}
 \frametitle{Example 1}
           {\bf Solution:} $W_{\text{rev}}$ is related to the area below the curve. Thus, we should use the $PV$ work equation:
           \begin{itemize}
              \item<1-> For process {\bf (a)}: 
                 \visible<1->{\begin{eqnarray}
                    d W = -PdV \Longrightarrow W &=& -P_{2}\left(V_{2}-V_{1}\right) \nonumber \\
                                                 &=& - 1\text{ atm}\left(2000-500\right)\text{ cm}^{3} = -1500\text{ atm.cm}^{3} \nonumber 
                 \end{eqnarray}}
%
                 \visible<2->{Now we need to convert {\it atm.cm}$^{3}$ to $J$, thus using the unit conversion table:}
                    \visible<2->{\begin{eqnarray}
...
\begin{frame}
 \frametitle{Example 1}
          \begin{itemize}
              \item<1-> For process {\bf (b)}, since $P$ is constant, i.e., $P_{1}=P_{2}$:
                 \visible<1->{\begin{eqnarray}
...
\begin{frame}
 \frametitle{Example 2}
           \blue{Calculate the internal energy (in $J$) when 1 mol of water is isobarically heated from 25$^{\circ}$C to 30$^{\circ}$C at 1 atm. Given: densities of water are 0.9970 g.cm$^{-3}$ at 0$^{\circ}$C and 0.9956 g.cm$^{-3}$ at 100$^{\circ}$C. Molar mass and heat capacity at constant pressure of water are 18 g.mol$^{-1}$ and 1 cal.$\left(\text{g.}^{\circ}\text{C}\right)^{-1}$, respectively.}\medskip

           \visible<2->{{\bf Solution:} From the 1$^{\text{st}}$ Law, $U=Q+W$ ,and in order to calculate $U$ we first need to obtain heat ($Q$) and work ($W$). $Q$ can be obtained from the heat capacity equation
          \begin{displaymath}
             Q = m C_{p} \Delta T,
          \end{displaymath}}  
          \visible<3->{where $m$ is the mass of water can be obtained from
          \begin{displaymath}
             n = \frc{m}{MW} \Longrightarrow  m = n.MW = 1\text{ mol} . 18 \frc{\text{g}}{\text{mol}} = 18 \text{ g}
          \end{displaymath}
          $n$ and $MW$ are number of moles and molar mass, respectively.} \visible<4->{Now,
          \begin{displaymath}
...
\begin{frame}
 \frametitle{Example 2}
          Now, we should calculate the work through $W=-P\Delta V$, however $V$ is not known, \visible<2->{but we can obtain it from the density relation $V=m/\rho$, thus
          \begin{eqnarray}
             W &=& - P\Delta V = -P\left(V_{2}-V_{1}\right) = -P\left(\frc{m}{\rho_{2}} - \frc{m}{\rho_{1}}\right)  \nonumber \\
               &=& -0.025\text{ atm.cm}^{3} \blue{\times \frc{1.01325\times 10^{5}\text{ Pa}}{1\text{ atm}} \times \frc{1\frc{\text{kg}}{\text{m.s}^{2}}}{1\text{ Pa}}\times} \nonumber \\
               &&   \hspace{1cm} \blue{\times \frc{1\text{ J}}{1\frc{\text{kg}}{\text{m.s}^{2}}} \times \frc{1\text{ m}^{3}}{100^{3}\text{ cm}^{3}} \times \frc{1\text{ cal}}{4.186\text{ J}}} \nonumber \\
               &=& -0.0006\text{ cal} \nonumber
          \end{eqnarray}}
          \visible<3->{Now, calculating the internal energy,
          \begin{displaymath}
...
\begin{frame}
 \frametitle{Example 3}
           \blue{Air initially occupying 1 m$^{3}$ at 1.5 bar and 20$^{\circ}$C undergoes an internally reversible compression for which $P V^{\gamma}=$ constant to a final state where the pressure is 6 bar and the temperature is 120$^{\circ}$C. Determine:}
        \begin{enumerate}[(a)]
           \item \blue{Value of $\gamma$;}
           \item \blue{Work and the heat transfer (in kJ).}
       \end{enumerate}
       \blue{Assume that heat capacity at constant volume and molar mass of air are 0.718 kJ.$\left(\text{kg.K}\right)^{-1}$ and 29 g.mol$^{-1}$. }
\medskip
        ...
        \end{center}}
        \begin{itemize}
            \item<3->In order to calculate the isentropic index $\gamma$, we can use any of the relations learned in the lecture,
               \begin{displaymath} 
...
\begin{frame}
 \frametitle{Example 3}
        \visible<1->{Although $P$ and $V$ is the relation initially given in the problem, we do not know the final volume of air, $V_{2}$. However, initial and final pressure and temperature are known and we can make use of this relationship to obtain $\gamma$,}
               \visible<2->{\begin{eqnarray} 
               ...
               \end{eqnarray}}
        \begin{itemize}
            \item<3->Compression work can be obtained by integrating $\d W = -P dV$ from state 1 to state 2 with $P=\frac{C}{V^{\gamma}}$,
...
\begin{frame}
 \frametitle{Example 3}
               \visible<1->{\begin{displaymath}
                 W_{1-2} =-\frc{P_{2}V_{2}^{\gamma}V_{2}^{1-\gamma} - P_{1}V_{1}^{\gamma}V_{1}^{1-\gamma}}{1-\gamma} = \frc{P_{1}V_{1}-P_{2}V_{2}}{1-\gamma} \nonumber
               \end{displaymath}}
               \visible<2->{The relation above, although important, can not be used as we do not know $V_{2}$, however we can change variables through the ideal gas relation}
               \visible<2->{\begin{eqnarray}
                  P V = n R T &\Rightarrow& n = \frc{P V}{R T } = \frc{1.5\text{ atm} \times 1\text{ m}^{3}}{ 8.3143 \frc{\text{J}}{\text{mol.K}} 293.15\text{ K}} \times \frc{1.01325\times 10^{5}\text{ Pa}}{1 \text{ atm}} \times \nonumber \\
                      &&   \times \frc{ 1 \text{ N.m}^{-2}}{1\text{ Pa}}\frc{1 \text{ J}}{1\text{ N.m}} = 62.3580\text{ moles}\nonumber \\
...
\begin{frame}
 \frametitle{Example 3}
               \visible<1->{Heat ($Q$) can be obtained from}
               \visible<2->{\begin{eqnarray}
          \Delta u &=& m C_{v}\Delta T = n MW C_{v}\Delta T = Q + W  \nonumber \\
             Q &=& n MW C_{v}\left(T_{2}-T_{1}\right) - W \nonumber \\
               &=& 62.3580\text{ mol}\times 29 \frc{\text{g}}{\text{mol}} \red{\frc{1\text{ kg}}{1000\text{ g}}}\times 0.718\frc{\text{kJ}}{\text{kg.K}}\times (393.15-293.15)\text{ K} \nonumber \\
               && \hspace{1cm} - 193.30 \text{ kJ} \nonumber \\
                                                          &=& -63.46\text{ kJ}. \nonumber                   
               \end{eqnarray}}
\end{frame}
