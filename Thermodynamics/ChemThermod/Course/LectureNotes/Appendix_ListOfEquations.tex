...
\chapter{Summary of the Main Equations}\label{Appendix:ListOfEquations}

%%% Chapters 1-4
\section{Fundamentals of Thermodynamics (Chapters~\ref{Chapter:Introduction}-\ref{Chapter:SecondLaw})}
  \begin{eqnarray}
...
\vspace*{\fill}
  ...
  %%% Chapter 5
\section{Volumetric Properties of Pure Substances}
  \begin{eqnarray}
...
\vspace*{\fill}
  ...
  %%% Chapter 6
\section{Thermodynamic Properties of Pure Fluids}
  \begin{eqnarray}
...
\vspace*{\fill}  
  ...
  %%% Chapter 7
\section{Vapour-Liquid Equilibrium of Mixtures}
  \begin{eqnarray}
...
\section{Chemical Reaction Equilibrium}
  \begin{eqnarray}
    ...
    && \summation[\nu_{i}G_{i}]{i}{} = \summation[\nu_{i}\mu_{i}]{i}{}=0;\;\;\; \prod\limits_{i}\left(\frc{\overline{f}_{i}}{f_{i}^{\circ}}\right)^{\nu_{i}} = \prod\limits_{i}a_{i}^{\nu_{i}} = K =\exp\left(\frc{-\Delta G^{\circ}}{RT}\right); \nonumber \\
    && \Delta H^{\circ} = -RT^{2}\frc{d}{dT}\left(\Delta G^{\circ}/RT\right)\;\;\;\text{ (Standard heat of reaction)};\nonumber \\
    && \frc{d\left(\ln{K}\right)}{dT} =\frc{\Delta H^{\circ}}{RT^{2}}\;\;\;\text{ (Van't Hoff equation)};\nonumber \\
    && \prod\limits_{i}\left(y_{i}\phi_{i}\right)^{\nu_{i}} =K \left(\frc{P}{P^{\circ}}\right)^{-\nu},\;\;\; \text{ where } \nu=\sum\limits_{i}\nu_{i}\;\;\text{ (gas-phase)}; \nonumber\\
    && \prod\limits_{i}\left(y_{i}\gamma_{i}\right)^{\nu_{i}} = K\exp\left[\frc{P^{\circ}-P}{RT}\sum\limits_{i}\left(\nu_{i}V_{i}\right)\right]^{-\nu}\;\;\text{ (liquid-phase)};\nonumber\\
    && \prod_{i}\left(y_{i}\right)^{\nu_{i,j}} = \left(\frc{P}{P^{\circ}}\right)^{-\nu_{i,j}}K_{j};\;\;\;\text{ (ideal gas multi-reaction)} \nonumber
  \end{eqnarray}
