...
\begin{frame}
 \frametitle{Example 3 }
    \blue{The van der Waals equation of state (vdW EOS),
        \begin{displaymath}
        ...
        can be expressed as a cubic polynomial equation in $Z$ (compressibility coefficient),
             \begin{displaymath}
                  Z^{3} -(1+B)Z^{2} +AZ -AB = 0,
             \end{displaymath}
             with $B=bP/(RT)$, $A=aP/(RT)^{2}$ and $R\left(=8.314\times 10^{-5}\frc{\text{bar.m}^{3}}{\text{mol.K}}\right)$. Calculate the fugacity of gaseous CO$_{2}$ at 310 K and 1.4 MPa using the vdW EOS, with $a=$ 0.3658 Pa.m$^{6}$.mol$^{-2}$, $b=$ 4.286$\times$10$^{-5}$ m$^{3}$.mol$^{-1}$. Given,
             \begin{displaymath}
                \ln{\left(\frc{f}{P}\right)} = -\ln{\left(1-\frc{b}{V}\right)} - \frc{a}{RTV} - \ln{Z} +\left(Z-1\right).
             \end{displaymath}
             Use the largest real root of the cubic polynomial equation in $Z$ to represent the gaseous phase.}
...
%\scriptsize 
\begin{frame}
 \frametitle{Example 8.3 (pg 143)}\label{Ex8_3}
...
%\scriptsize 
\begin{frame}
 \frametitle{Example 8.4 (pg 144)}\label{Ex8_4}
      ...
      \item<1-> 2-parameter models for binary systems:
        \begin{enumerate}
          ...
          \item<2-> Van Laar equations:
             \visible<2->{\begin{displaymath}
                \ln\gamma_{1}= B_{12}\left(1+\frc{B_{12}x_{1}}{A_{21}x_{2}}\right)^{-2}  \hspace{1cm} \ln\gamma_{2}= B_{21}\left(1+\frc{B_{21}x_{2}}{A_{12}x_{1}}\right)^{-2}
             \end{displaymath}}
...
%\scriptsize 
\begin{frame}
 \frametitle{Example 5}\label{Ex5}
   \blue{Plot $P-x-y$ and $x-y$ phase diagrams for a mixture of ethyl acetate and benzene at 75$^{\circ}$C. Given}
     \begin{displaymath}
       \blue{\ln{P_{i}^{\text{sat}}}  = A_{i} - \frc{B_{i}}{T-C_{i}}, with \begin{cases}A_{1}=9.6830; B_{1}=2842.2; C_{1}=56.3209, & \\ A_{2}=9.3171; B_{2}=2810.5; C_{2}=51.2586 & \end{cases}} 
     \end{displaymath}
     \blue{with [P] in bar and [T] in K. The Van Laar model has coefficients $B_{12}=A_{12}=1.15$ and $B_{21}=A_{21}=0.92$} 
