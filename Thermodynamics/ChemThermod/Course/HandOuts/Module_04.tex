...
\begin{frame}
 \frametitle{Example 8.2 (pg 139)}\label{Ex8_1}
    \blue{For the acetone (Ket, 1) - methanol (MetOH, 2) system, a vapour mixture of $z_{\text{Ket}}$ = 0.25 and $z_{\text{MetOH}}$ = 0.75 is cooled to temperature $T$ in the two-phase region and flows into a separation chamber at a pressure of 1 bar. If the composition of the liquid product is $x_{\text{Ket}}$ = 0.175, \underline{calculate $T$  and $y_{\text{Ket}}$}. For the liquid mixture, assume that
\begin{displaymath}
\ln\gamma_{1} = 0.64x_{2}^{2} \;\;\;\;\;\text{ and }\;\;\;\;\;\ln\gamma_{2}=0.64x_{1}^{2}
\end{displaymath}
For the Antoine equation, 
\begin{displaymath}
\ln P_{i}^{\text{sat}} = A_{i} - \frc{B_{i}}{T + C} \hspace{1cm} \text{ with [P] = kPa and [T] = }^{\circ}\text{C}
\end{displaymath}
\begin{center}
\begin{tabular}{c| c c c}
                &   {\bf A}   & {\bf B}                         & {\bf C} \\
                &             & $\left(^{\circ}\text{C}\right)$   & $\left(^{\circ}\text{C}\right)$ \\
\hline
{\bf Acetone (1)}   &   14.3145   &   2756.22                       &  228.060                      \\
{\bf Methanol (2)}  &   16.5785   &   3638.27                       &  239.50                       \\
\end{tabular}
\end{center}}
\end{frame}
...
\end{displaymath} 
\underline{Determine $T$ and $P$} for the separator. Given the activity coefficients for the liquid phase,
\begin{displaymath}
\ln\gamma_{1} = 0.93x_{2}^{2} \;\;\;\;\;\text{ and }\;\;\;\;\;\ln\gamma_{2}=0.93x_{1}^{2},
\end{displaymath}
and
\begin{displaymath}
\ln P_{i}^{\text{sat}} = A_{i} - \frc{B_{i}}{T} \hspace{1cm} \text{ with [P] = bar and [T] = K},
\end{displaymath} 
