...
%\scriptsize
\begin{frame}
  \frametitle{Reaction Coordinate for Multi-Reactions Systems}
  \begin{block}{\begin{center}General Chemical Reaction\end{center}}
        Given the reaction in ultiple steps:
        \begin{displaymath}
           \nu_{a}A(g) + \nu_{b}B(g) \Leftrightarrow \nu_{c}C(g) + \nu_{d}D(g) \begin{cases}
               \nu_{a,1}A(g) \leftrightarrow \nu_{c,1}C(g) + \nu_{e,1}E(g) & \\
               \nu_{e,2}E(g) + \nu_{b,2}B(g) \leftrightarrow \nu_{f,2}F(g) & \\
               \nu_{a,3}A(g) + \nu_{f,3}F(g) \leftrightarrow \nu_{d,3}D(g) & \\
           \end{cases}
         \end{displaymath} 
  \end{block}
