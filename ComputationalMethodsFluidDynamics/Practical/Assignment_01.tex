...
\item\label{Practical1:Task2} {\bf Turbulent Free Jet}
  \begin{enumerate} 
     ...
     \item Use the standard $\kappa-\epsilon$ turbulence model. The $\kappa-\epsilon$ terms must be specified on all boundaries; 
     ...
     \item At the {\it Inlet 1} windows, modify the {\it Specification Method} for {\it Turbulence} to \blue{\it Intensity and Hydraulic Diameter}. The intensity should be $I=0.037942$, and the hydraulic diameter should be $d_{H}=0.01 m$;
     ...
     \item In the {\it `Monitors'}, edit the residual monitors and ensure that the residual criteria for both $\kappa$ and $\epsilon$ are 10$^{-6}$;
     \item Repeat steps \red{(ii)}-\red{(iv)} from~\ref{Practical1:Task1};
     \item Plot the turbulent kinetic energy along the centreline, i.e., $\kappa\times\mathbf{X}$;
...
A group of engineers and consultants are designing a new industrial unit near the coast. Cold sea water is used as cooling fluid for all heat exchangers and condensers of the unit, and then returned back to the sea through a pipeline of XXX of diameter ($\overline{EF}$ in Fig.~\ref{EG501V_Assignment:Sketch2}) at XXX kg/s (properties of wasted and sea water are assumed the same, $\rho=$ XXX kg/m$^{3}$, $\mu=$ XXX cP). 
