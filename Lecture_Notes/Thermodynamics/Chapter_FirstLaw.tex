...
%%%
     ...
     \begin{subequations}
         ...
         In most thermodynamic systems, changes in kinetic and potential energies are often assumed negligible\footnote{Here, we are assuming that such systems are not movable \wrt frames of reference.}, and Eqn.~\ref{Chapter:FirstLaw:Eqn:TotalEnergy3} becomes
            \begin{equation}
               \mfr[E]{2}{\text{cv}} - \mfr[E]{1}{\text{cv}} = U_{2}-U_{1} = Q_{1-2} + W_{1-2},\label{Chapter:FirstLaw:Eqn:FirstLaw1}
            \end{equation}
...
%%%% SUBSECTION
       ...
       \begin{shaded}
          ...
          \end{equation}
          $\eta_{\text{max}}$ is also known as the {\it Carnot efficiency}\index{Carnot Efficiency} and it is the maximum efficiency any power cycle can achieve while operating between two reservoirs.
       \end{shaded}
...
% SOLUTION
   ...
   % Example
   \begin{MyExample}{\begin{center}{\bf Example}\end{center}}
     \begin{example}\label{Chapter:FirstLaw:Example5}\citep{Atkins_Book}
   ...
   \begin{MyExample}{\begin{center}{\bf Example}\end{center}}
     \begin{example}\label{Chapter:FirstLaw:Example6}\citep{Rajput_Book}
       ...
       \begin{displaymath}
         PV = C,
       \end{displaymath}
       where [$P$] = bar and [$V$] = m$^{3}$, and $C$ = 3 bar.m$^{3}$ Calculate the work (in kJ) done during the compression.
     \end{example}
...
%%%
   ...
   In the previous sections, the path-dependence of the work added or produced,
   \begin{displaymath}
        W=-\int PdV,
   \end{displaymath}
 was a result of the different ways to compress/expand fluids, represented by the area under a curve in a $PV$ diagram (Fig.~\ref{Chapter:FirstLaw:Fig:Work_PV}). The area under the curve defined by path $A$ is clearly different from the one defined under the curve of path $B$, in fact
   \begin{displaymath}
     W_{12}^{A} > W_{12}^{B},
   \end{displaymath}
