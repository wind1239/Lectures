              ...
              Now using Eqn.~\ref{Chapter:SolutionThermodynamics:Eqn:GibbsDuhem1k} to calculate the partial molar volume $\overline{V}_{i}$,
                 \begin{eqnarray}
                    \overline{V}_{1} &=& V + x_{2}\frc{dV}{dx_{1}} = V + x_{2}\frc{d}{dx_{1}} \left(\overbrace{\Delta V}^{V^{E}} + x_{1}V_{1} + x_{2}V_{2}\right) \nonumber \\
                         &=& V + x_{2}\frc{d}{dx_{1}} \left[x_{1}x_{2}\left(45x_{1}+25x_{2}\right)+x_{1}V_{1}+x_{2}V_{2}\right] = 124.76\text{ cm}^{3}.\text{mol}^{-1}, \nonumber% 190.28\text{ cm}^{3}.\text{mol}^{-1}, \nonumber
                 \end{eqnarray}
                 with the same procedure for $\overline{V}_{2}=93.36\text{ cm}^{3}.\text{mol}^{-1}$.% 49.68\text{ cm}^{3}.\text{mol}^{-1}$.
                 We can check if our calculations are correct with,
                 \begin{displaymath}
...
\medskip
     ...
     \begin{displaymath}
          \Delta_{\text{mix}}\overline{S}^{\text{igm}} = \summation[n_{i}\left(\overline{S}_{i}^{\text{igm}}(T,P,y) - \overline{s}_{i}^{\text{ig}}(T,P) \right) ]{i=1}{\mathcal{C}} = -R\summation[n_{i}\ln{y_{i}}]{i=1}{\mathcal{C}}.
     \end{displaymath}
...
%
      In  Section~\ref{Chapter:SolutionThermodynamics:Section:FugacitySection} (Eqn.~\ref{Chapter:SolutionThermodynamics:Eqn:fugacity1b}), a formal definition of fugacity was introduced,
       ...
       \begin{shaded}
         ...
         \begin{eqnarray}
           \mfr[\overline{\phi}]{i}{V} &=& \frc{\mfr[\overline{f}]{i}{V}}{y_{i}P}\;\;\text{ and, }\label{Chapter:SolutionThermodynamics:Eqn:FugCoeffVap1}\\
           \mfr[overline{\phi}]{i}{L} &=& \frc{\mfr[\overline{f}]{i}{L}}{x_{i}P_{i}^{\text{sat}}}.\label{Chapter:SolutionThermodynamics:Eqn:FugCoeffLiq1}
         \end{eqnarray}
       \end{shaded}
