...
\begin{frame} % Holman pg 143
 \frametitle{Criteria for Lumped System --  Example 1}
      A steel ball of 5 cm in diameter and at uniform temperature of 450$^{\circ}$C is suddenly placed in a controlled environment where temperature is kept at 100$^{\circ}$C.  The prescribed convection heat transfer coefficient is 10 W.$\left(\text{m}^{2}.^{\circ}\text{C}\right)^{-1}$. Calculate the time required for the ball to reach 150$^{\circ}$C. Given:

\begin{flushleft}
 C$_{p,\text{steel}} = 0.46\text{ kJ.kg}^{-1}$ \\
 $\kappa_{\text{steel}}=35\text{ W.}\left(\text{m.}^{\circ}\text{C}\right)^{-1}$ and \\
$\rho_{\text{steel}}=7.8\times 10^{3}\text{kg.m}^{-3}$.
\end{flushleft}
\end{frame}
...
%%%
\begin{frame}
 \frametitle{Analytical Methods: Example2}
