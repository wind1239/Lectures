...
\blue{The shaft is then allowed to cool slowly in an environment chamber at 200$^{\circ}$C with an average heat transfer coefficient of ($h$) of 80 W/$\left(\text{m}^{2}.^{\circ}\text{C}\right)$.}
\medskip
...
\visible<1->{As the problem deals with a long geometry and the thermal symmetry is in the centerline, we can assume the problem as a 1D. }\visible<2->{ The radius of the cylinder is 0.1 m, thus}
    \begin{displaymath}
